...
res_group <- fread("output/deseq2/asw/interaction/exposure_ruakura/res_group.csv")
...
fwrite(annot_sig_fgsea, "output/deseq2/asw/interaction/exposure_ruakura/sig_GO_enrichment.csv")

##list genes in leading edge of term
longest_isoform_trinotate <- fread("data/asw-transcriptome/output/trinotate/sorted/longest_isoform_annots.csv")
####Core members that contribute to ES score (present in list before running sum reaches max.dev. from 0)
term_res <- annot_sig_fgsea[annot_sig_fgsea$pathway == "GO:0003676",]
term_leading_edge <- data.frame(term_res$leadingEdge)
setnames(term_leading_edge, old=c("term_res.leadingEdge"), new=c("gene_id"))
##cannot get this list into a table to merge with transcriptome annotations
term_leading_edge_list <- tstrsplit(term_leading_edge$gene_id, "|", fixed=TRUE)
term_leading_edge_annots <- subset(longest_isoform_trinotate, `#gene_id` %in% term_leading_edge_list)
fwrite(term_leading_edge_annots, "output/deseq2/asw/interaction/exposure_ruakura/DNA_binding_leading_edge_annots.csv")

######annot_sig_fgsea <- fread("output/deseq2/asw/exposed/sig_GO_enrichment.csv") -- do i need this?

#####################
## all in one plot ##
#####################
plot_fgsea <- fread("output/deseq2/asw/interaction/exposure_ruakura/sig_GO_enrichment_edited_plot.csv")
###swap _ for space in pathway kind
plot_fgsea$pathway_kind <- gsub("_", " ", plot_fgsea$pathway_kind)
##reorder - sorts by pathway_kind reverse alphabetically but can't figure out how to do any better
plot_fgsea$pathway_name <- factor(plot_fgsea$pathway_name,
                                       levels=plot_fgsea$pathway_name
                                       [order(plot_fgsea$pathway_kind, plot_fgsea$NES, decreasing=TRUE)])
##plot
ggplot(plot_fgsea, aes(pathway_name, NES)) +
  geom_segment(aes(y=0, yend=plot_fgsea$NES, x=pathway_name, xend=pathway_name), alpha=0.4)+
