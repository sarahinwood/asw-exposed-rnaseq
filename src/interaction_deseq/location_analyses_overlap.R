...
location <- fread('output/deseq2/asw/location/sig_w_annots.csv')
...
##################################
## RuEx, InvEx, NC_Loc analyses ##
##################################

##Venn diagram of overlap
vd1 <- venn.diagram(x = list("effect of location on all ASW"=location$rn,
                             "effect of exposure on Ruakura ASW"=exposure_ruakura$rn,
                             "effect of exposure on Invermay ASW"=exposure_invermay$rn),
                    fill=c("#440154FF", "#FDE725FF", "#21908CFF"), filename=NULL,
                    alpha=0.7, cex = 1, cat.cex=1, lwd=1.5)
grid.newpage()
grid.draw(vd1)
ggsave(file="output/deseq2/asw/venn_diagrams/rex_iex_location.svg", plot=vd1, width=5, height=4)


##overlap gene lists
##location specific
a <- setdiff(location$rn, exposure_ruakura$rn)
loc_nc_sp <- setdiff(a, exposure_invermay$rn)
loc_nc_sp_annots <- subset(trinotate_blast, `#gene_id` %in% loc_nc_sp)

#exposure_ruakura specific
b <- setdiff(exposure_ruakura$rn, location$rn)
...
c <- setdiff(exposure_invermay$rn, location$rn)
...
locnc_ruex <- intersect(location$rn, exposure_ruakura$rn)
locnc_ruex_annots <- subset(trinotate_blast, `#gene_id` %in% locnc_ruex)

##location nc vs invermay exposed
locnc_invex <- intersect(location$rn, exposure_invermay$rn)
...
annot_counts <- filter(counts_table, rn %in% ruex_inex)
