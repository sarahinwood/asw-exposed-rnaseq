...
vd2 <- venn.diagram(x = list("location specific repsonse to exposure"=location_exposure$rn,
                             "location-exposure interaction"=interaction$rn), fill=c("#440154FF", "#FDE725FF"), filename=NULL, alpha=0.7, cex = 1, cat.cex=1, lwd=1.5)
grid.newpage()
grid.draw(vd2)
...
shared_int <- setdiff(interaction$rn, interaction)
